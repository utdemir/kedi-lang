...
pub fn run(fun: &plain::Module) -> simple::Module {
    let statements = fun.statements.iter().map(simplify_top_level_stmt).collect();
...
fn simplify_block(state: &mut SimplifyFunImplState, instrs: &mut Instrs, stmt: &[plain::FunStmt]) {
...
impl SimplifyFunImplState {
    ...
    fn compile_expr(&mut self, instrs: &mut Instrs, expr: &plain::Expr) -> simple::Ident {
        match expr {
            ...
            plain::Expr::Ident(id) => self.widen_plain_ident(id),
